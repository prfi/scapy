...
# Workaround for windows imports issue
        ...
        class IPsphere(visual.sphere):
            ...
            def setlabel(self, txt, visible=None):
                if self.label is not None:
                    ...
                    visible = 0
                self.label = visual.label(text=txt, pos=self.pos, space=self.radius,
                                          xoffset=10, yoffset=20, visible=visible)
...
def IPID_count(lst, funcID=lambda x: x[1].id, funcpres=lambda x: x[1].summary()):
    ...
    idlst.sort()
    classes = [idlst[0]] + map(lambda x: x[1], filter(lambda (x, y): abs(x - y) >
                                                      50, map(lambda x, y: (x, y), idlst[:-1], idlst[1:])))
